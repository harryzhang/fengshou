package com.kder.business.action.login;
...
import com.kder.business.common.IResult;
import com.kder.business.common.ResultSupport;
...
public class LoginController extends BaseAction {
    ...
    public void resetLoginUserPwd(HttpServletRequest request, HttpServletResponse response) {
        try{
    		...
    		ManagersDo managerDo = managerUserService.getUserById(userId);
    		//判断密码是否相等
    		Md5PasswordEncoder encoder = new Md5PasswordEncoder();
            String passwordEncode = encoder.encodePassword(oldPassword,managerDo.getSalt());
      		int ret = -1;
            if (managerDo.getPassword().equals(passwordEncode)) {
            	newPassword = encoder.encodePassword(newPassword.trim(),managerDo.getSalt());
      			ret = managerUserService.resetCurrentUserPwd(userId, oldPassword, newPassword);
      		}
