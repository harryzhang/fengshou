...
public class AdminController extends BaseAction {
     ...
     * @param user
     */
    private void encodePassword(ManagersDo user){
    	Md5PasswordEncoder encoder = new Md5PasswordEncoder();
    	String salt = RandomStringUtils.randomAlphanumeric(6);
		user.setSalt(salt);
		user.setPassword(encoder.encodePassword(user.getPassword().trim(), salt));
    }
    ...
    public void resetLoginUserPwd(HttpServletRequest request,
        ...
        try{
            ...
            ManagersDo managerDo = managerUserService.getUserById(Integer.valueOf(userId));
    		//判断密码是否相等
    		Md5PasswordEncoder encoder = new Md5PasswordEncoder();
            String passwordEncode = encoder.encodePassword(newPassword.trim(),managerDo.getSalt());
            int ret=managerUserService.resetUserPwd(Integer.valueOf(userId),  passwordEncode);
//            if (ret > 0) {
//                //记录密码更新时间
