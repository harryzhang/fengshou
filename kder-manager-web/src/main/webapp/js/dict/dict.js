...
/*##########################grid init begin###################################################*/
	...
	$("#tt").datagrid({
		url:httpUrl+"/dict/listDict.html?&rand=" + Math.random(),
...
function to_edit(id){
	...
	var url = httpUrl+"/dict/addDict.html?&rand=" + Math.random()+"&id="+id;
	$('#editDiv').dialog({
		title: "新增字典",
		width: 760,
		height: 500,
...
function save_dict(){
	...
	console.info(formdata);
	var  url =httpUrl+"/dict/saveDict.html?&rand=" + Math.random();
	 $.ajax({   
		 type: 'POST',
		 dataType: 'json',
		 url: url,  
